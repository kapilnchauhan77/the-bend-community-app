...
Final race/interceptor fix commit SHA: `4c9f04ff8761dabdb506bd363058a1d3bef3d219`.

Initialization-race fix commit SHA: `82a8376296c90734a41549a7f880c27da2c325dc`.

Review fixes add the authenticated tenant-scoped `/auth/me` endpoint, manager bearer/tenant headers, refresh/logout race protection, null-refresh cleanup, runtime-derived encoded WebSocket URLs, allowlisted pending-destination integration, and expanded regression tests. Second-review fixes preserve pending destinations through failed login/rerenders, invalidate stale refresh rejection after newer login, and expose a testable Axios client factory. Final fixes serialize secure-store/browser mutations and test suspended-save logout ordering plus thrown-refresh redirect injection. Initialization fixes coalesce concurrent hydration and ensure stale load/refresh/me failures or responses cannot clear or overwrite a newer login/logout. Backend focused and full suite: `pytest -q tests/test_auth_me.py` — 2 passed; `pytest -q` — 82 passed. Frontend focused/regression suite — 9 files, 50 tests passed; lint, web build, and native build passed. No push or deployment performed.
