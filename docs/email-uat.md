...
`EmailService._send` selects Resend when `RESEND_API_KEY` is configured, then SendGrid when only `SENDGRID_API_KEY` is configured. With neither key, it writes a development-only log and returns `False`. The provider response only establishes provider acceptance. It does not establish inbox delivery, spam placement, or link correctness. No provider key, credential, customer address, or synthetic secret belongs in this document.
...
| E5. Listing interest notification | An authenticated member expresses interest in an active listing; sends to the listing owner's user email. | `send_interest_notification_email`; subject includes the listing title; owner greeting, listing title, and `/my-shop` link. | [`interest_service.py`](../the-bend-backend/app/services/interest_service.py#L21), [`ListingDetailPage.tsx`](../the-bend-frontend/src/pages/ListingDetailPage.tsx#L140), [`email_service.py`](../the-bend-backend/app/services/email_service.py#L189). |
...
- A2. Password reset: request a reset for an existing synthetic account. Confirm the generic response, one message, a deployed non-localhost URL, and one-time token use. Request an unknown address and confirm the same response with no message.
- A3. Listing interest: express interest from a second synthetic user in the first user's active listing. Confirm one owner message and one in-app notification. Repeat or withdraw and confirm no duplicate send.
...
- A5. Provider modes: in non-production, test Resend configured, SendGrid-only configured, and neither configured. Confirm precedence and that no-key mode logs only a development message.
