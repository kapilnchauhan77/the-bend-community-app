...
class Report(Base):
    ...
    target_type: Mapped[str] = mapped_column(
        String(16), nullable=False, default="listing"
    )
    ...
    resolved_by_platform_admin_id: Mapped[uuid.UUID | None] = mapped_column(
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ["reporter_id", "tenant_id"],
            ["users.id", "users.tenant_id"],
            ondelete="RESTRICT",
        ),
        ForeignKeyConstraint(
            ["resolved_by_id", "tenant_id"],
            ["users.id", "users.tenant_id"],
            ondelete="SET NULL",
        ),
        CheckConstraint(
            "resolved_by_id IS NULL OR resolved_by_platform_admin_id IS NULL",
            name="ck_reports_single_resolver",
        ),
    )
