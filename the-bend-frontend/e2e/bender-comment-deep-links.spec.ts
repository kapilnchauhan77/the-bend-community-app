...
test('routes a Bender reply notification to the exact post and comment query string', async ({ page }) => {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (decodeURIComponent(url.pathname) === '/api/v1/bender/posts/post 1/comments') return route.fulfill({ contentType: 'application/json', body: JSON.stringify({ items: [], next_cursor: null, has_more: false }) });
    if (decodeURIComponent(url.pathname) === '/api/v1/bender/posts/post 1/comments/reply/1') return route.fulfill({ status: 404, body: 'missing' });
    if (decodeURIComponent(url.pathname) === '/api/v1/bender/posts/post 1') return route.fulfill({ contentType: 'application/json', body: JSON.stringify({ ...post, id: 'post 1' }) });
    return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({}) });
  });
  const directPostResponse = page.waitForResponse((response) => decodeURIComponent(new URL(response.url()).pathname) === '/api/v1/bender/posts/post 1');
  const directCommentResponse = page.waitForResponse((response) => decodeURIComponent(new URL(response.url()).pathname) === '/api/v1/bender/posts/post 1/comments/reply/1');
  await page.goto('/notifications');
  await page.getByText('Reply').click();
  await directPostResponse;
  await directCommentResponse;
  await expect(page.getByRole('heading', { name: 'Bender' })).toBeVisible();
  await expect(page.locator('[data-testid="bender-post"]')).toHaveAttribute('id', 'post-post 1');
  await expect(page.getByTestId('bender-comments-drawer')).toContainText('No comments yet. Be the first.');
