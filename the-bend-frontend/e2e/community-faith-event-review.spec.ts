...
test('community submission requires coupon and sends exact free payload', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /Submit/ }).click();
  await expect.poll(() => payload).toEqual(expect.objectContaining({ organization_type: 'community_faith', is_nonprofit: false, coupon_code: 'COMMUNITY100' }));
});
...
test('verified nonprofit submission sends exact payload after document upload', async ({ page }) => {
  ...
  await fillRequired(page);
  await page.getByRole('button', { name: /Submit/ }).click();
  await expect.poll(() => payload).toEqual(expect.objectContaining({ organization_type: 'verified_nonprofit', is_nonprofit: true, nonprofit_doc_url: '/uploads/proof.pdf' }));
});
...
test('for-profit submission sends exact payload and retains optional coupon', async ({ page }) => {
  ...
  await expect.poll(() => payload).toEqual(expect.objectContaining({ organization_type: 'for_profit', is_nonprofit: false, coupon_code: 'PROMO20' }));
});
