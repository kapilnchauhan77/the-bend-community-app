...
test('calendar keeps the current title aligned with its image, count, accessible date, long-title containment, and reduced motion', async ({ page }) => {
  ...
  const day = page.getByRole('button', { name: new RegExp(`Morning river cleanup.*${dateLabel}`) });
  ...
  await page.reload();
  await page.getByRole('button', { name: 'Calendar', exact: true }).click();
  const reducedDay = page.getByRole('button', { name: new RegExp(`Morning river cleanup.*${dateLabel}`) });
