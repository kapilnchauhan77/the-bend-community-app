    ...
    <>
      <nav aria-label="Native app navigation" className="native-bottom-nav native-safe-bottom fixed inset-x-0 bottom-0 z-50 border-t bg-white pb-[env(safe-area-inset-bottom)] shadow-lg">
        <div className="native-bottom-nav-inner mx-auto flex h-16 max-w-lg items-center justify-around">
          ...
          <button ref={postTriggerRef} type="button" aria-label="Create" onClick={() => { void haptics.impact(); setPostOpen(true) }} className="flex min-w-14 flex-col items-center gap-1 p-2 text-xs">
          ...
          {actions.slice(2).map(({ label, path, icon: Icon }) => (
            <button key={label} type="button" aria-label={label} aria-current={location.pathname.startsWith(path) ? 'page' : undefined} onClick={() => select(label, path)} className="flex min-w-14 flex-col items-center gap-1 p-2 text-xs">
              <Icon size={20} aria-hidden="true" />
              <span>{label}</span>
            </button>
          ))}
