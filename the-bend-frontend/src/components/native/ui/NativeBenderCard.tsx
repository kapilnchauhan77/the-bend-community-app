...
export function NativeBenderCard({ post, onOpen }: NativeBenderCardProps) {
  ...
  return <button type="button" className="native-bender-card native-control" aria-label={getBenderAccessibleName(displayName, caption)} onClick={() => onOpen(`/bender?post=${encodeURIComponent(post.id)}`)}>
