...
export function NativeDiscoveryCard({ item, onOpen }: NativeDiscoveryCardProps) {
  ...
  return <button type="button" aria-label={accessibleName} className={`native-discovery-card native-${item.kind}-discovery-card native-control`} onClick={() => onOpen(item.targetPath)}>{item.thumbnailUrl && failedThumbnailUrl !== item.thumbnailUrl ? <img src={resolveAssetUrl(item.thumbnailUrl)} alt={item.kind === 'bender' ? '' : item.title} width="96" height="96" loading="lazy" onError={() => setFailedThumbnailUrl(item.thumbnailUrl)} /> : <div className="native-thumbnail native-thumbnail-fallback" data-fallback-icon={item.kind} aria-hidden="true"><FallbackIcon size={28} /></div>}<span><small>{item.label}</small><strong>{item.title}</strong><span>{item.supportingText}</span></span></button>
}
