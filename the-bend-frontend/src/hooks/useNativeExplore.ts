...
import { adaptBender, adaptBusiness, adaptEvent, adaptListing, adaptOpportunity } from '@/native/discovery/adapters'
...
const headings: Record<NativeDiscoveryKind, string> = { listing: 'Listings', business: 'Businesses', event: 'Events', bender: 'Bender', volunteer: 'Volunteer' }
...
export function useNativeExplore(query: NativeExploreQuery): NativeExploreViewModel {
  ...
  const benderFetcher = useCallback(async () => items(await benderApi.listPosts(undefined, 5, { search: requestQuery.q || undefined })), [requestQuery.q])
  ...
  const bender = useCachedPublicContent(key('bender', 'bender:native-explore-default'), benderFetcher, { enabled: all, cachePolicy: 'none' })
  ...
  const refreshBender = bender.refresh
  ...
  const groups: NativeExploreGroup[] = [
    ...
    { kind: 'bender', heading: headings.bender, state: { status: bender.status, data: empty(bender.data).slice(0, 5).map(adaptBender), source: bender.source, cachedAt: bender.cachedAt, error: bender.error, retry: bender.refresh } },
...
export function sortBusinessesByDistance(items: NativeDiscoveryCardModel[], userCoordinates: { latitude: number; longitude: number }, hydrated: Record<string, { latitude: number; longitude: number }>) {
  return items.map((item, index) => {
    const coordinates = item.coordinates ?? hydrated[item.id]
    return { item, index, distance: coordinates ? haversine(userCoordinates.latitude, userCoordinates.longitude, coordinates.latitude, coordinates.longitude) : null }
  }).sort((left, right) => {
