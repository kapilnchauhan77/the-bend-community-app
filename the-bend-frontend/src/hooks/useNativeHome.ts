...
import type { SuccessStory, Sponsor, Listing, CommunityEvent } from '@/types'
...
export function useNativeHome(now?: Date) {
  ...
  return useMemo(() => ({ urgent: section(urgent), upcoming: section(upcoming), opportunities: section(opportunities), highlights: section(highlights), partners: section(partners) }), [urgent, upcoming, opportunities, highlights, partners])
}
