...
function normalizeEventUuid(value: string | undefined): string | null { return parseCanonicalUuid(value ?? '')?.toLowerCase() ?? null; }
...
export function EventDetailPage() {
  const { eventId } = useParams<{ eventId: string }>(); const services = usePlatformServices(); const native = useNativePresentation(); const [state, setState] = useState<EventDetailState>({ status: 'loading' }); const [retryKey, setRetryKey] = useState(0);
  const nativeRouteLinkClass = native ? 'native-control native-route-link' : undefined;
  const canonicalEventId = normalizeEventUuid(eventId);
  const load = useCallback(() => { if (!canonicalEventId) { setState({ status: 'unavailable' }); return () => undefined; } const controller = new AbortController(); let active = true; setState({ status: 'loading' }); eventApi.getDetail(canonicalEventId, { signal: controller.signal }).then((response) => { const responseEventId = normalizeEventUuid(response.data.id); if (active && !controller.signal.aborted) { if (responseEventId !== canonicalEventId) { setState({ status: 'error', error: new Error('Event response did not match requested event') }); return; } setState({ status: 'success', event: response.data }); } }).catch((error: unknown) => { if (!active || controller.signal.aborted) return; const status = responseStatus(error); if (status && [400, 401, 403, 404, 422].includes(status)) setState({ status: 'unavailable' }); else setState({ status: 'error', error: error instanceof Error ? error : new Error('Could not load event') }); }); return () => { active = false; controller.abort(); }; }, [canonicalEventId]);
  ...
  const isCurrentSuccess = state.status === 'success' && normalizeEventUuid(state.event.id) === canonicalEventId;
  let content: React.ReactNode;
  if (state.status === 'loading' || (state.status === 'success' && !isCurrentSuccess)) content = <p role="status" className="event-detail-message">Loading event</p>;
  else if (state.status === 'unavailable') content = <section role="status" className="event-detail-message"><h1>Event unavailable</h1><p>This event is no longer available.</p><Link to="/events" className={nativeRouteLinkClass}>Back to events</Link></section>;
