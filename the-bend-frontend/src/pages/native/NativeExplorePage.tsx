...
const chips: Array<[string, NativeExploreType]> = [['All', 'all'], ['Listings', 'listings'], ['Businesses', 'businesses'], ['Events', 'events'], ['Bender', 'bender'], ['Volunteer', 'volunteer']]
...
export function NativeExplorePage() {
  ...
  return <div className="native-explore-scroll" role="region" aria-label="Explore content">
    ...
    <div className="native-explore-controls">{mapAllowed && <button type="button" disabled={query.mode === 'list' && (model.online !== true || mapBusinesses.length === 0)} onClick={() => change({ mode: query.mode === 'list' ? 'map' : 'list' })}>{query.mode === 'list' ? 'Map' : 'List'}</button>}{query.type === 'businesses' && <button type="button" onClick={() => { if (!query.near) void requestCurrentLocation(true); else change({ near: false }) }}>{query.near ? 'Near me on' : 'Near me'}</button>}{filtersAllowed && <button ref={trigger} type="button" onClick={() => setSheet(true)}>Filters</button>}</div>
