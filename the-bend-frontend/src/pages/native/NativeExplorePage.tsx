...
export function NativeExplorePage() {
  ...
  const requestCurrentLocation = () => { void model.requestLocation() }
  return <div className="native-explore-scroll" role="region" aria-label="Explore content">
    ...
    <div className="native-explore-controls">{mapAllowed && <button type="button" disabled={model.online === false || (query.mode === 'list' && mapBusinesses.length === 0)} onClick={() => change({ mode: query.mode === 'list' ? 'map' : 'list' })}>{query.mode === 'list' ? 'Map' : 'List'}</button>}{query.type === 'businesses' && <button type="button" onClick={() => { if (!query.near) requestCurrentLocation(); change({ near: !query.near }) }}>{query.near ? 'Near me on' : 'Near me'}</button>}<button ref={trigger} type="button" onClick={() => setSheet(true)}>Filters</button></div>
    ...
    {model.location && model.location.status !== 'idle' && model.location.status !== 'granted' && query.type === 'businesses' && <section role="status"><p>Location permission is needed only for Near me.</p><button type="button" onClick={requestCurrentLocation}>Retry</button><button type="button" onClick={() => change({ near: false })}>Continue across Westmoreland</button></section>}
    {query.mode === 'map' && mapAllowed ? <section role="status"><h2>Business map</h2>{model.online === false ? <p>Map is unavailable offline. Continue with the list.</p> : mapBusinesses.length === 0 ? <p>No businesses with public map coordinates are available yet.</p> : <Suspense fallback={<p>Loading map…</p>}><NativeExploreMap businesses={mapBusinesses} userCoordinates={model.userCoordinates ?? null} selectedId={mapSelected} onSelect={setSelectedMapId} onOpen={(path) => navigate(path)} /></Suspense>}{query.type === 'all' && <p>Map scope: businesses only in Westmoreland.</p>}<button type="button" onClick={requestCurrentLocation}>Use my location</button></section> : query.type === 'all' ? model.groups.map((group) => <NativeResultGroup key={group.kind} heading={group.heading} status={group.state.status} onRetry={group.state.retry} onSeeAll={() => change({ type: group.kind === 'listing' ? 'listings' : group.kind === 'business' ? 'businesses' : group.kind === 'event' ? 'events' : 'volunteer' })}>{group.state.data.map(card)}</NativeResultGroup>) : model.typed && <NativeResultGroup heading={chips.find(([, type]) => type === query.type)?.[0] ?? 'Results'} status={model.typed.state.status} onRetry={model.typed.state.retry}>{model.typed.state.data.map(card)}{model.typed.hasMore && <button type="button" onClick={() => void model.typed?.loadMore()} disabled={model.typed.loadingMore}>Load more</button>}{model.typed.refineMessage && <p>{model.typed.refineMessage}</p>}{model.typed.loadMoreError && <p role="alert">Unable to load more results.</p>}</NativeResultGroup>}
