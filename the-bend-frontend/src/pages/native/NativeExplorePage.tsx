...
export function NativeExplorePage() {
  const [params, setParams] = useSearchParams(); const query = parseNativeExploreQuery(params); const requestedMapMode = params.get('mode') === 'map'; const queryRef = useRef(query); const canonicalKey = serializeNativeExploreQuery(query).toString(); const [text, setText] = useState(query.q); const [sheet, setSheet] = useState(false); const [selectedMapId, setSelectedMapId] = useState<string | null>(null); const [locationIntent, setLocationIntent] = useState<'map' | 'near'>(query.near ? 'near' : 'map'); const [locationPrimerDismissed, setLocationPrimerDismissed] = useState(false); const trigger = useRef<HTMLButtonElement>(null); const timer = useRef<number | null>(null); const navigate = useNavigate(); const model = useNativeExplore(query)
  useEffect(() => {
  ...
  const requestCurrentLocation = async (intent: 'map' | 'near' = 'map') => { setLocationIntent(intent); setLocationPrimerDismissed(false); const result = await model.requestLocation(); if (intent === 'near') change({ near: result.status === 'granted' }) }
  const locationFailed = model.location && (model.location.status === 'denied' || model.location.status === 'unavailable')
  const showLocationPrimer = locationFailed && !locationPrimerDismissed && ((locationIntent === 'near' && query.type === 'businesses') || (locationIntent === 'map' && query.mode === 'map'))
  const dismissLocationPrimer = () => { setLocationPrimerDismissed(true); if (locationIntent === 'near') change({ near: false }); else change({ mode: 'list' }) }
  useEffect(() => {
  ...
  return <div className="native-explore-scroll" role="region" aria-label="Explore content">
    ...
    <div className="native-explore-controls">{mapAvailability.status === 'available' && <NativeViewModeSwitch value={query.mode} onChange={(mode) => change({ mode })} />}{query.type === 'businesses' && <button type="button" onClick={() => { if (!query.near) void requestCurrentLocation('near'); else change({ near: false }) }}>{query.near ? 'Near me on' : 'Near me'}</button>}{filtersAllowed && <button ref={trigger} type="button" onClick={() => setSheet(true)}>Filters</button>}</div>
    ...
    {showLocationPrimer && <PermissionPrimer title={locationIntent === 'near' ? 'Use your location for Near me' : 'Use your location on the map'} description={model.location.message} confirmLabel="Retry" onConfirm={() => void requestCurrentLocation(locationIntent)}><button type="button" className="native-control" onClick={dismissLocationPrimer}>Continue across Westmoreland</button></PermissionPrimer>}
    <div id="native-explore-results" role="tabpanel" aria-labelledby={`native-explore-tab-${query.type}`}>
    {query.near && query.type === 'businesses' && <><h2>Near you within Westmoreland</h2>{(!model.userCoordinates || (model.typed?.state.data.some((item) => item.kind === 'business') && model.mapBusinesses.length < model.typed.state.data.filter((item) => item.kind === 'business').length)) && <p role="status">{model.mapBusinesses.length === 0 ? 'Distance unavailable for businesses; showing the current server order.' : 'Distance unavailable for some businesses; they remain in stable server order after distance-sorted businesses.'}</p>}</>}{query.mode === 'map' && mapAvailability.status === 'available' ? <section aria-labelledby="native-business-map-title"><h2 id="native-business-map-title">Business map</h2><p id="native-map-location-help">We ask only after you choose this button. Your location sorts or centers nearby Westmoreland results.</p><Suspense fallback={<p>Loading map…</p>}><NativeExploreMap businesses={mapBusinesses} userCoordinates={model.userCoordinates ?? null} selectedId={mapSelected} onSelect={setSelectedMapId} onOpen={(path) => navigate(path)} /></Suspense>{query.type === 'all' && <p>Map scope: businesses only in Westmoreland.</p>}<button type="button" className="native-map-location-control" aria-describedby="native-map-location-help" onClick={() => void requestCurrentLocation('map')}>Use my location</button></section> : query.type === 'all' ? model.groups.map((group) => <NativeResultGroup key={group.kind} heading={group.heading} status={group.state.status} onRetry={group.state.retry} onSeeAll={() => change({ type: group.kind === 'listing' ? 'listings' : group.kind === 'business' ? 'businesses' : group.kind === 'event' ? 'events' : group.kind === 'bender' ? 'bender' : 'volunteer' })}><CachedContentNotice cachedAt={group.state.cachedAt} />{group.state.data.map(card)}</NativeResultGroup>) : model.typed && <NativeResultGroup heading={NATIVE_EXPLORE_TYPES.find(([, type]) => type === query.type)?.[0] ?? 'Results'} status={model.typed.state.status} onRetry={model.typed.state.retry}>{model.typed.state.data.map(card)}{model.typed.hasMore && <button type="button" onClick={() => void model.typed?.loadMore()} disabled={model.typed.loadingMore}>Load more</button>}{model.typed.refineMessage && <p>{model.typed.refineMessage}</p>}{model.typed.loadMoreError && <p role="alert">Unable to load more results.</p>}</NativeResultGroup>}
    </div>
    {filtersAllowed && <NativeFilterSheet open={sheet} closeLabel="Close filters" onClose={() => setSheet(false)} returnFocusRef={trigger}><h3>Filter options</h3>{filterChoices.map((value) => <button key={value} type="button" onClick={() => { change({ category: value }); setSheet(false) }}>{value}</button>)}{(query.type === 'listings' || query.type === 'volunteer') && <><button type="button" onClick={() => { change({ urgency: 'urgent' }); setSheet(false) }}>urgent</button><button type="button" onClick={() => { change({ urgency: 'normal' }); setSheet(false) }}>normal</button></>}{(query.type === 'listings' || query.type === 'volunteer') && sorts.map((value) => <button key={value} type="button" onClick={() => { change({ sort: value }); setSheet(false) }}>{value}</button>)}<button type="button" onClick={() => { change({ category: null, urgency: null, sort: null }); setSheet(false) }}>Clear filters</button></NativeFilterSheet>}
  </div>
}
